...
class CrudElasticTag(CrudElasticBase[TagElastic]):
    def __init__(
        self,
...
def iter_elastic_query(
    ...
    count = 0
    dsl = {"size": size, "query": query, "track_total_hits": True, "sort": ["_doc"]}
    hits = elastic_client.search(index=index, body=dsl, scroll=scroll)
    hits = SearchResult(**hits)
    count += len(hits.hits.hits)
    yield hits
    while hits.hits.hits:
        hits = elastic_client.scroll(scroll_id=hits.scroll_id, scroll=scroll)
        hits = SearchResult(**hits)
        count += len(hits.hits.hits)
        yield hits
...
class CrudTag:
    ...
    async def insert(self, tag: TagInsert) -> TagInsert:
        ...
        if tag.id is not None:
            old_tag = self.get_row_by_id_by_elastic(tag.id)
    ...
    def get_row_by_id_by_elastic(self, tag_id: int) -> TagElastic:
        try:
            hit = self.elastic_client.get(index=self.index, id=tag_id)
    ...
    async def get_row_by_id(self, tag_id: int) -> TagUpdate:
        ...
        if token is None:
            return None
        elastic_tag = self.get_row_by_id_by_elastic(tag_id)
    ...
    async def get_interpretation_by_id(self, tag_id: int) -> Tag:
        tag_in_ids = self.get_row_by_id_by_elastic(tag_id)
    ...
    def _delete_elastic(self, tag_id: int):
        query = {
            "bool": {
                "should": [
        ...
        batches = []
        for hits in iter_elastic_query(
            self.elastic_client, self.index, self.size, query
        ):
            for hit in hits.hits.hits:
                ...
                if len(batches) > self.batch_size:
                    helpers.bulk(self.elastic_client, batches)
                    batches = []
        if len(batches) > 0:
            helpers.bulk(self.elastic_client, batches)
    ...
    async def delete_by_id(self, tag_id: int) -> bool:
        ...
        self._delete_elastic(tag_id)
        try:
            self.elastic_client.delete(index=self.index, id=tag_id)
        except NotFoundError:
            pass
        return True
