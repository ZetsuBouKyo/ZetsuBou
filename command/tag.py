...
class Tag:
    """Operations for Tag in ZetsuBou."""

    @sync
    async def dump(self, out: str, force: bool = False):
        out = Path(out)
        if not out.is_dir():
            return
        ...
        async def get_attributes():
            ...
            while rows:
                ...
                skip += limit
                rows = await CrudTagAttribute.get_rows_order_by_id(
    ...
    @sync
    async def load(self, attr: str, tag: str):
        async def is_ok():
