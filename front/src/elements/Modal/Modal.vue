<template>
  <div class="flex flex-col bg-opacity-0 z-50" :class="position" v-if="state.popout">
    ...
    <div
      class="z-50 flex flex-col w-full h-full bg-gray-800 rounded-lg shadow-black animate-scale-in-center rounded-lg"
    >
      ...
      <div
        class="flex flex-col w-full h-full py-1"
        :class="isScrollable ? 'overflow-y-scroll scrollbar-gray-100-2' : ''"
      >
        <slot></slot>
      </div>
    </div>
