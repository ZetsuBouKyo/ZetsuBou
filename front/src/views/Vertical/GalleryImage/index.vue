...
function plotPoint(event: MouseEvent) {
    ...
    let transformedX =
        cos * x - sin * y + (state.container.imgWidth * state.container.scale) / 2 / state.container.scale;
    let transformedY =
    ...
    if (state.sidebar.polygon.polygons[currentID]) {
        ...
        state.sidebar.polygon.polygons[currentID] = {
            id: currentID,
            points: [point],
            isVisible: true,
            isCompleted: false,
...
const draw = () => {
    if (
        !canvas.value ||
        !ctx.value ||
        state.container.imgWidth === undefined ||
        state.container.imgHeight === undefined
    )
        return;
